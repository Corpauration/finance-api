...
object Event {
  given Meta[JsonObject] =
    Meta.Advanced.other[PGobject]("jsonb").timap[JsonObject](
      a => decode[JsonObject](a.getValue).leftMap[JsonObject](e => throw e).merge)(
      a => {
        val o = new PGobject
        o.setType("jsonb")
  ...
  given Read[Event] = Read[(UUID, OffsetDateTime, JsonObject)].map(Event.apply)
    ...
    event.data
      ...
      .map {
        ...
        => AccountEvent.AccountDeactivated(
          accountId, reason, labels, timestamp
        )
